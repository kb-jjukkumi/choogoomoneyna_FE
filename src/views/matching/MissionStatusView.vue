<template>
  <TopNavigation />
  <div class="relative flex flex-col min-h-[calc(100vh-120px)] bg-ivory mt-15">
    <!-- 매칭 참가자 프로필 -->
    <div class="flex justify-around px-10 items-center mb-7">
      <!-- 나 -->
      <div class="flex flex-col items-center">
        ...
        <div class="text-xs text-limegreen-900 mt-2">
          {{ MATCHING_DATA.user1.nickname }}
        </div>
        <div class="text-xs text-gray-300">
          {{ MATCHING_DATA.user2.score }}점
        </div>
      </div>
    ...
    >
      <p class="text-lg text-limegreen-900 pt-5 pb-3 px-2">미션 목록</p>
      ...
      <div class="mt-auto mb-8">
        <button
          class="w-full bg-ivory text-limegreen-800 py-2.5 rounded-xl text-sm font-semibold"
          @click="goToStatus"
        >
          미션 현황
        </button>
...
const goToStatus = () => {
  router.push({
    name: 'missionStatus',
    params: {
      matchingId: MATCHING_DATA.value.matchingId,
    },
    query: {
      data: encodeURIComponent(JSON.stringify(MATCHING_DATA.value)),
    },
  });
};
</script>
